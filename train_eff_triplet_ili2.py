...
def train(rank, a, h):
     ...
     for epoch in range(max(0, last_epoch), a.training_epochs):
          ...
          if ILI:
               ...
               n_batches=int(len(trainset_support)/h.batch_size),
               alpha=balance_alpha,
               kind='random'
               )
               #############################
               train_loader_support = DataLoader(trainset_support, num_workers=h.num_workers, batch_sampler=batch_sampler, pin_memory=True)
               train_loader_query = DataLoader(trainset_query, num_workers=h.num_workers, batch_sampler=None, pin_memory=True)

          for i, batch in enumerate(train_loader_support):
               if rank == 0:
                    start_b = time.time()
               ...
               if rank == 0:
                    ...
                    if steps % a.checkpoint_interval == 0 and steps != 0:
                         ...
                         if ILI:
                              LABELS = plainILI2(a.checkpoint_path, h, LABELS, train_loader_query)
                         model.train()
