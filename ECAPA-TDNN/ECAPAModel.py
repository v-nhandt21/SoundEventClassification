...
class ECAPAModel(nn.Module):
	...
	def eval_network(self, eval_list, eval_path):
		...
		for line in lines:
			files.append(os.path.join(eval_path, line))
		...
		for idx, file in tqdm.tqdm(enumerate(setfiles), total = len(setfiles)):
			...
			event_label = dictkeys[file.split("/")[-1].split("-")[0]]
			...
			_, _, predict1       = self.speaker_loss.forward(embedding_1, torch.LongTensor([0]).cuda())

			

			y = torch.nn.functional.one_hot( torch.tensor(event_label), num_classes=8).unsqueeze(0)

			predict1 = torch.nn.Softmax(dim=1)(predict1)

			Y_pred.append(predict1.detach().cpu().numpy())
			Y_target.append(y.detach().cpu().numpy())

		Y_pred = numpy.concatenate(Y_pred, axis=0 )
		Y_target = numpy.concatenate( Y_target, axis=0 )

		print(Y_pred.shape)
		print(Y_target.shape)

		report(Y_target, Y_pred, list(dictkeys.keys()), "test_confusion_matrix.png")
